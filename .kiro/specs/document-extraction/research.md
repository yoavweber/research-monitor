...
- **Implications**:
  ...
  - **Sample verification done in Task 2.3 (2026-04-29)**: ran `extraction_mineru_adapter_test.go` against the AMM-arbitrage fixture; the design's normalizer contract aligns with reality on every rule. **414 inline `$...$`** spans, **348 display `$$...$$`** spans, **0 occurrences** of `\(...\)` or `\[...\]` (the defensive rewrite path is dead for this MinerU 3.1 host but kept for future config drift). `## References` heading present at line 739 of the bundle markdown — case-insensitive exact-match rule fires. `![](images/<hash>.jpg)` image lines present (12) in the format the design assumed. `Figure N:` figure-caption prefixes match design's prefix pattern. **No tables** in this paper (MinerU produces them when source has them). The first `#` heading carries the document title verbatim. Adapter test now passes (110s elapsed including model cold-start) with assertions ratcheted to the adapter's pass-through contract; the e2e test in Task 5.2 will assert the post-normalize invariants.
